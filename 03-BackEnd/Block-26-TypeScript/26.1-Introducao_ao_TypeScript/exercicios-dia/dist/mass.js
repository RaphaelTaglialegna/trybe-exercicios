...
function makeErrorMass(unity) {
    throw new Error(`A unidade ${unity} não é uma unidade válida.`);
}
function convertMass(value, forUnity, toUnity) {
    if (!mass.includes(forUnity))
        makeErrorMass(forUnity); // se a unidade base não for válida lançamos um erro
    if (!mass.includes(toUnity))
        makeErrorMass(toUnity); // se a unidade para a conversão não for válida lançamos um erro
