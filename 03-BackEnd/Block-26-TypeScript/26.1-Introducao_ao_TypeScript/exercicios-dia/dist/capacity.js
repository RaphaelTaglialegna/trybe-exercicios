...
function makeErrorLiquid(unity) {
    throw new Error(`A unidade ${unity} não é uma unidade válida.`);
}
function convertLiquid(value, forUnity, toUnity) {
    if (!liquid.includes(forUnity))
        makeErrorLiquid(forUnity); // se a unidade base não for válida lançamos um erro
    if (!liquid.includes(toUnity))
        makeErrorLiquid(toUnity); // se a unidade para a conversão não for válida lançamos um erro
