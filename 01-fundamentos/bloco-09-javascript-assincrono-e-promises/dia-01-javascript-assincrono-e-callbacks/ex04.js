...
const sendMarsTemperature = (delay, callback) => {
  const mensage = callback(getMarsTemperature);
  const timeDelay = delay(messageDelay);
  setTimeout(() => console.log(`Mars temperature is: ${mensage}  degree Celsius`), timeDelay);
};
