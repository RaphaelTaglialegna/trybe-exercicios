...
const sendMarsTemperature = (mensage, callbackError) => {
  const mensagens = getMarsTemperature();
  const rendomNumber = Math.random();
  const sendSuccessfully = rendomNumber < 0.6;
  setTimeout(() => {
    if (sendSuccessfully) {
      mensage(mensagens)
    } else {
      callbackError('Tivemos um pequeno problema aqui!')
    }
    }, messageDelay());  
